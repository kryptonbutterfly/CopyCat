...
cp $SOURCE_DIR/target/$APP_NAME-$PACKAGE_VERSION.jar $BUILD_DIR/usr/share/$PACKAGE_NAME/$APP_NAME.jar
...
cd /tmp
dpkg --build debian
mv /tmp/debian.deb $SOURCE_DIR/build/$APP_NAME-$PACKAGE_VERSION.deb
rm -r $BUILD_DIR

cp $SOURCE_DIR/build/$APP_NAME-$PACKAGE_VERSION.deb $DEPLOY_DIR/Deb/
cp $SOURCE_DIR/target/$APP_NAME-$PACKAGE_VERSION.jar $DEPLOY_DIR/$APP_NAME.jar
